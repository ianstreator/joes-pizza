const Appetizers = {
  Quantity: ["6pc", "12pc", "24pc"],
  quantityAppetizers: {
    "Jumbo Buffalo Wings": [
      ["8.99", "11.99", "22.99"],
      "Famous meaty wings hand-dipped in your choice of mild, medium, hot BBQ or garlic sauce, served with a side of blue cheese or ranch.",
    ],
    "Chicken Strips": [
      ["7.25", "13.00", "23.00"],
      "Home-style battered and served regular or buffalo with blue cheese or ranch.",
    ],
    "Fried Ravioli": [
      ["8.00", "14.00", "25.00"],
      "Fried golden brown and served with our homemade marinara.",
    ],
    "Mozzarella Sticks": [
      ["6.25", "10.25", "18.25"],
      "Creamy mozzarella breaded and served with marinara.",
    ],
    "Jalepeno Poppers": [
      ["6.75", "11.25", "20.00"],
      "Spicy Jalapenos stuffed with cream cheese. Served with marinara or ranch.",
    ],
  },
  soloAppetizers: {
    "Onion Rings": [5.99, "Mound of crispy rings, enough to share."],
    Calamari: [
      9.75,
      "An Italian classic, succulent squid deep fried to perfection in an Italian-style batter.",
    ],
    "Stuffed Mushrooms (6pc)": [
      9.75,
      "Fresh mushrooms fried in a tasty golden breading. Crab filling 100%.",
    ],
    "Breaded Cauliflower": [
      6.25,
      "Delicately nutty, fried until irresistible.",
    ],
    "Breaded Zucchini": [
      6.25,
      "Sweet garden-fresh zucchini in a golden breading.",
    ],
    "Breaded Mushrooms": [
      6.25,
      "Fresh mushrooms fried in a tasty golden breading.	",
    ],
    "Veggie Combo": [
      9.25,
      "Eating your veggies is easy (and tasty) when they're this good! A plentiful mix of mushrooms, cauliflower, and zucchini.	",
    ],
    "Tour De Joe's": [
      11.5,
      "The perfect appetizer combination! Jumbo buffalo wings, mozzarella sticks, spicy jalapeno poppers, and breaded mushrooms mean something for everyone!	",
    ],
    "Breadsticks (4pc / 8pc)": [[3.5, 6.0], "Buttery goodness!"],
    "Fried Dough Nuggets": [
      9.75,
      "Tossed in a garlic parmigiana butter, they're just right the perfect bite.",
    ],
    "Garlic Bread": [4.5, "French bread with garlic and butter."],
    "Cheezy Garlic Bread": [
      6.25,
      "French bread topped with garlic, butter, and melted mozzarella.",
    ],
    "Pizza Bread": [
      6.25,
      "French bread topped with garlic, butter, tomato sauce and real mozzarella.        ",
    ],
    "Ultimate Fries": [
      8.75,
      'When we say "ultimate" we mean it! These crispy golden wonders are smothered with cheddar cheese and crispy bacon.',
    ],
    "French Fries": [3.75, "Crispy and golden brown."],
  },
};

export default Appetizers;
